 ...
 * 类上面的注解是用来设置配置文件中的标签。
 */
@Plugin(name = "Jmx", category = "Core", elementType = Appender.ELEMENT_TYPE, printObject = true)
public class JmxAppender extends AbstractAppender {
    ...
    // 需要实现的构造方法，直接使用父类就行
    protected JmxAppender(final String name, final Filter filter, final Layout<? extends Serializable> layout,
    ...
    public void append(LogEvent event) {
        ...
        try {
            ...
            Log4j2Manager.getInstance().pushLogger(JacksonUtil.objToJson(loggerMessage));
        } catch (Exception ex) {
            if (!ignoreExceptions()) {
    ...
    @PluginFactory
    public static JmxAppender createAppender(@PluginAttribute("name") String name, @PluginElement("Filter") final Filter filter,
        ...
        try {
            ...
            mbeanServer.registerMBean(Log4j2Manager.getInstance(), mbean);
        ...
        return new JmxAppender(name, filter, layout, ignoreExceptions);
    }
