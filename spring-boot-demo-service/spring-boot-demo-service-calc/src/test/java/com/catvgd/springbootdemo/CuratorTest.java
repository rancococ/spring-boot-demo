...
public class CuratorTest {
    ...
    public CuratorTest() {
        zkTools = CuratorFrameworkFactory.builder().connectString("192.168.8.200:2181").namespace("zktest").retryPolicy(new RetryNTimes(2000, 20000))
                .build();
        zkTools.start();
    }
    ...
    public void register() throws Exception {
        ...
        zkTools.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL).withACL(ZooDefs.Ids.OPEN_ACL_UNSAFE).forPath(registeNode, data);// 创建的路径和值
        // 添加到session过期监控事件中
